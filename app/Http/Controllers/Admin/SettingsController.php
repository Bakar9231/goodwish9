...
class SettingsController extends Controller
{
	...
	public function sendNotification(Request $request)
	{
		...
		// 	$notification_img = $request->file('notification_img');
		// 	$imagename = 'notification_' . rand(111,999) . '.' . $notification_img->getClientOriginalExtension();
		// 	$destinationPath = public_path('uploads/');
		// 	File::makeDirectory($destinationPath, $mode = 0777, true, true);
		// 	$notification_img->move($destinationPath, $imagename);
	...
	public function UpdateNotification(Request $request)
	{
		...
		// 	$data['icon'] = $imagename;
		// }else{
		// 	$data['icon'] = $request->input('hidden_notification_img');
		// 	$notification_image = url(env('DEFAULT_IMAGE_URL').$request->input('hidden_notification_img'));
		// }

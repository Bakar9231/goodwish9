...
class ContentController extends Controller
{
	...
	public function viewListContent()
	{
		...
		return view('admin.content.content_list')->with('total_content', $total_content)->with('total_movie', $total_movie)->with('total_series', $total_series)->with('total_realtiyshow', $total_realtiyshow);
	}
	...
	public function addUpdateContent(Request $request)
	{
		...
		if ($request->hasfile('verticle_poster')) {
			...
			$imageFileName = 'content_' . rand(111, 999) . '.' . $file->getClientOriginalExtension();
			$destinationPath = public_path('uploads/');
			File::makeDirectory($destinationPath, $mode = 0777, true, true);
			$file->move($destinationPath, $imageFileName);
			$verticle_poster = $imageFileName;
		} else {
		...
		if ($request->hasfile('horizontal_poster')) {
			...
			$imageFileName2 = 'content_' . rand(111, 999) . '.' . $file->getClientOriginalExtension();
			$destinationPath = public_path('uploads/');
			File::makeDirectory($destinationPath, $mode = 0777, true, true);
			$file->move($destinationPath, $imageFileName2);
			$horizontal_poster = $imageFileName2;
		} else {
	...
	public function UploadContentSourceMedia(Request $request)
	{
		...
		if ($request->hasfile('source_video')) {
			$source_video = $request->file('source_video');
			$imageFileName = 'source_' . rand(111, 999) . '.' . $source_video->getClientOriginalExtension();
			$destinationPath = public_path('uploads/');
			File::makeDirectory($destinationPath, $mode = 0777, true, true);
			$source_video->move($destinationPath, $imageFileName);
		}
	...
	public function addUpdateContentSubtitles(Request $request)
	{
		...
		if ($request->hasfile('subtitle_file')) {
			...
			$imageFileName = 'subtitle_' . rand(111, 999) . '.' . $subtitle_file->getClientOriginalExtension();
			$destinationPath = public_path('uploads/');
			File::makeDirectory($destinationPath, $mode = 0777, true, true);
			$subtitle_file->move($destinationPath, $imageFileName);
			$data['subtitle_file'] = $imageFileName;
		}
	...
	public function addUpdateSeasonEpisode(Request $request)
	{
		...
		if ($request->hasfile('episode_thumb')) {
			$episode_thumb = $request->file('episode_thumb');
			$imageFileName = 'episode_' . rand(111, 999) . '.' . $episode_thumb->getClientOriginalExtension();
			$destinationPath = public_path('uploads/');
			File::makeDirectory($destinationPath, $mode = 0777, true, true);
			$episode_thumb->move($destinationPath, $imageFileName);
			$data['episode_thumb'] = $imageFileName;
		}

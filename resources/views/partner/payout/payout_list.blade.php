...
<div class="modal fade" id="partnerEditModal" tabindex="-1" role="dialog" aria-labelledby="ModalLabel" aria-hidden="true">
  <div class="modal-dialog" role="document">
    <div class="modal-content">
      <div class="modal-header">
        <h5 class="modal-title" id="ModalLabel"> Update Payout Request </h5>
        <button type="button" class="close" data-dismiss="modal" aria-label="Close">
          <span aria-hidden="true">&times;</span>
        </button>
      </div>
      <form id="updatepartner" method="post" enctype="multipart">
        ...
        <div class="modal-body">
          <div class="form-group">
            <label for="name">Amount</label>
            <input id="upd-amount" name="amount" type="text" class="form-control form-control-danger" placeholder="Enter Payout">
          </div>
      ...
      function(isConfirm) {
        if (isConfirm) {
          ...
          $.ajax({
            ...
            success: function(data) {
              if (data.success == 1) {
                swal("PayOut Rrquest Deleted Succfully", "", "success");

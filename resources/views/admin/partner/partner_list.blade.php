      ...
      <div class="col-12">
        <div class="card">
          <div class="card-header">
            <h4 class="m-0">Partner's List (<span class="total_partners">
                {{ $total_partners }}
              </span>)</h4>
          </div>
          ...
          <div class="card-body">
            ...
            <div class="table-responsive">
              ...
              <table id="partner" class="table table-striped table-sm display">
                <div class="pull-right">
                ...
                <tbody>
                  ...
                  <tr>
                    ...
                    <img height="60" width="60" src="{{ URL::asset($detail->profileImg) }}">
                    @else
                    <img height="60" width="60" src="{{ asset('assets/dist/img/default.png') }}">
                    @endif
                    </td>
                    <td>{{ $detail->name }}</td>
                    <td>{{ $detail->email }}</td>
                    ...
                    <td>{{ $detail->referral_link }}</td>
                    <td>
                      @if($detail->status == 1)
                      <label class="switch">
                        <input id="partnerStatus" data-id="{{ $detail->id }}" onclick="updateStatus(0,'<?php echo  $detail->id ;?>')" type="checkbox" value="0" checked>
                        <span class="slider round"></span>
                      </label>
                      @else
                      <label class="switch">
                        <input id="partnerStatus" data-id="{{ $detail->id }}" onclick="updateStatus(1,'<?php echo  $detail->id ;?>')" type="checkbox" value="1">
                        <span class="slider round"></span>
                      </label>
                      @endif
                    </td>
                    <td>{{ $detail->referredUsers }}</td>
...
<div class="modal fade" id="partnerEditModal" tabindex="-1" role="dialog" aria-labelledby="ModalLabel" aria-hidden="true">
  <div class="modal-dialog" role="document">
    <div class="modal-content">
      <div class="modal-header">
        <h5 class="modal-title" id="ModalLabel"> Update Partner </h5>
        <button type="button" class="close" data-dismiss="modal" aria-label="Close">
          <span aria-hidden="true">&times;</span>
        </button>
      </div>
      <form id="updatepartner" method="post" enctype="multipart">
        ...
        <div class="modal-body">
          <div class="form-group">
            <label for="name">Partner Name</label>
            <input id="upd-name" name="name" type="text" class="form-control form-control-danger" placeholder="Enter Partner Name">
          </div>

          <div class="form-group">
            <div class="input-group">
              <input id="upd-referral_income_percentage" name="referral_income_percentage" type="text" class="form-control form-control-danger" placeholder="Enter Referral Percentage" min="10" max="100" onkeypress="return onlyNumberKey(event)">
              <label for="referral_income_percentage" style="flex: 0.1 !important; font-size: large;" class="form-control form-control">%</label>
            </div>
          </div>
          ...
          <div class="form-group">
            <label for="email">Email</label>
            <input id="upd-email" name="email" type="email" class="form-control form-control-danger" placeholder="Enter Partner Email">
          </div>

          <div class="form-group">
            <label for="mobile">Partner Mobile</label>
            <input id="upd-mobile" name="mobile" type="text" class="form-control form-control-danger" onkeypress="return onlyNumberKey(event)" placeholder="Enter Partner Mobile Number">
          </div>

          <div class="form-group">
            <label for="password">Password</label>
            <input id="upd-password" name="password" type="password" class="form-control form-control-danger" autocomplete="off" placeholder="Enter Password">
          </div>

          <div class="form-group">
            <input id="referral_code" name="referral_code" type="hidden" value="<?php echo ("ref" . rand(1, 20) . date('d') . rand(1, 20)) ?>" class="form-control form-control-sm">
            <input id="upd-referral_link" name="referral_link" type="hidden" value="https://goodwish9.com/partner/" class="form-control form-control-sm">
          </div>
    ...
    $.ajax({
      ...
      success: function(data) {
        if (data.success == true) {
          ...
          $("#upd-id").val(data.partner.id);
          $("#upd-name").val(data.partner.name);
          $("#upd-referral_income_percentage").val(data.partner.referral_income_percentage);
          $("#upd-email").val(data.partner.email);
          $("#upd-mobile").val(data.partner.mobile);
          if(data.partner.profile_image){
            $("#upd-img-1").show();
            $("#upd-img-1").attr("src", '<?php echo asset('/uploads'); ?>' +'/' + data.partner.profile_image );
          }
         
          console.log(data.partner.name);
...
<script>
  ...
  $(document).ready(function() {
        ...
        $.ajax({
          ...
          success: function(data) {
            ...
            if (data.success == 1) {
              ...
              window.location = '{{ route("/partner/list") }}';
            } else {
              iziToast.error({
                title: 'Error!',
                message: data.message,
    ...
    $(document).on('submit', '#addUpdatePartner', function(e) {
      e.preventDefault();
      let user_type = 1;
      let referralCode = $('#referral_code');
      let referralLink = $('#referral_link');

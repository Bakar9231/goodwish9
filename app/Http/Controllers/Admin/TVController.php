...
class TVController extends Controller
{
	...
	public function addUpdateTVCategory(Request $request){
		...
		if ($request->hasfile('category_image')) {
			...
			$imageFileName = 'category_' . rand(111,999) . '.' . $category_image->getClientOriginalExtension();
			$destinationPath = public_path('uploads/');
			File::makeDirectory($destinationPath, $mode = 0777, true, true);
			$category_image->move($destinationPath, $imageFileName);
	...
	public function addUpdateTVChannel(Request $request){
		...
		if ($request->hasfile('channel_thumb')) {
			$channel_thumb = $request->file('channel_thumb');
			$imageFileName = 'channel_' . rand(111,999) . '.' . $channel_thumb->getClientOriginalExtension();
			$destinationPath = public_path('uploads/');
			File::makeDirectory($destinationPath, $mode = 0777, true, true);
			$channel_thumb->move($destinationPath, $imageFileName);
			$data['channel_thumb'] = $imageFileName;
		}
